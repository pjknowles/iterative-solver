...
//  for (size_t i = 0; i < n; ++i, ++ix) {
//    if (ix->first < x.size())
//      selection.emplace(max ? (ignore_sign ? abs((ix->second)) : (ix->second)) : (ignore_sign ? -abs((ix->second)) : -(ix->second)), ix->first);
//  }
//  for (auto jx = ix; jx != end(x); ++jx) {
//    if (jx->first < x.size()) {
//      selection.emplace(max ? (ignore_sign ? abs((ix->second)) : (ix->second)) : (ignore_sign ? -abs((ix->second)) : -(ix->second)), jx->first);
//      selection.pop();
//    }
//  }
//  auto selection_map = std::map<size_t, value_type>();
