      ...
      do while (Iterative_Solver_End_Iteration_Needed())
        if (nwork.gt.0) then
          if (use_diagonals) then
            ...
            if (IterativeSolverHasEigenvalues().ne.0) then
              call problem%precondition(actions_(:, :nwork), shift = Iterative_Solver_Working_Set_Eigenvalues(nwork), &
                  diagonals = parameters_(:, 1), range = Iterative_Solver_Range())
            else
              call problem%precondition(actions_(:, :nwork), &
                  diagonals = parameters_(:, 1), range = Iterative_Solver_Range())
            end if
          else
