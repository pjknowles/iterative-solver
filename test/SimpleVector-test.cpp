...
TEST(SimpleVector, copy_constructor) {
  using pv = linalg::SimpleVector<double>;
  pv v0(10001);
...
TEST(SimpleVector, dot_product) {
  linalg::SimpleVector<double> v0(10001);
  for (size_t i = 0; i < v0.size(); i++) v0[i] = 2 * i + 1;
...
TEST(SimpleVector, scal) {
  linalg::SimpleVector<double> v0(10000);
  for (size_t i = 0; i < v0.size(); i++) v0[i] = 2 * i + 1;
  bool result = true;
  for (auto i = 0; i < 4; i++) {
    auto v1 = linalg::SimpleVector<double>(v0, i);
    auto v2 = linalg::SimpleVector<double>(v0, i);
    v2.scal(3);
    v2.axpy(-3, v1);
//         std::cout << v2.dot(v2) <<std::endl;
    result &= v2.dot(v2) < 1e-20;
...
linalg::SimpleVector<double> v0(10001, i);
