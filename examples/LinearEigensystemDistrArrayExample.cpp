...
int main(int argc, char* argv[]) {
  ...
  auto solver = molpro::linalg::itsolv::create_LinearEigensystem<Rvector, molpro::linalg::array::DistrArrayFile>("Davidson");
  solver->set_n_roots(argc > 2 ? std::stoi(argv[2]) : 2);
  auto problem = ExampleProblem(argc > 1 ? std::stoi(argv[1]) : 20, argc > 3 ? std::stoi(argv[3]) : solver->n_roots());
  ...
  for (size_t i = 0; i < problem.m_vectors.size(); i++) {
    problem.m_vectors[i].assign(problem.n, 0);
    problem.m_vectors[i][i] = 1;
