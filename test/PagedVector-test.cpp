...
TEST(PagedVector_test, copy_constructor) {
  using pv = molpro::linalg::PagedVector<double, 100>;
  pv v0(10001);
...
TEST(PagedVector, pass_through) {
  ...
  auto v1 = molpro::linalg::PagedVector<double>(v.data(), v.size());
  auto& w = v1;
  double r = 0;
  bool ad = true;
  for (size_t i = 0; i < v.size(); ++i) {
...
TEST(PagedVector, dot) {
  molpro::linalg::PagedVector<double> v0(10001);
  for (size_t i = 0; i < v0.size(); i++) v0[i] = 2 * i + 1;
...
TEST(PagedVector, scal) {
  molpro::linalg::PagedVector<double> v0(10000);
  for (size_t i = 0; i < v0.size(); i++) v0[i] = 2 * i + 1;
  bool result = true;
  for (auto i = 0; i < 4; i++) {
    auto v1 = molpro::linalg::PagedVector<double>(v0, i);
    auto v2 = molpro::linalg::PagedVector<double>(v0, i);
    v2.scal(3);
    v2.axpy(-3, v1);
//         std::cout << v2.dot(v2) <<std::endl;
    result &= v2.dot(v2) < 1e-20;
...
molpro::linalg::PagedVector<double> v0(10001, i);
