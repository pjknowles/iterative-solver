...
TEST(SimpleVector, scal) {
  ...
  for (auto i = 0; i < 4; i++) {
    auto v1 = LinearAlgebra::SimpleVector<double>(v0, i);
    auto v2 = LinearAlgebra::SimpleVector<double>(v0, i);
    v2.scal(3);
    v2.axpy(-3, v1);
//         std::cout << v2.dot(v2) <<std::endl;
    result &= v2.dot(v2) < 1e-20;
...
//  std::cout << "option "<<i<<std::endl;
    ...
    LinearAlgebra::SimpleVector<double> v0(10001, i);
