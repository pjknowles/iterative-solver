...
void OldDIIS::setOptions(size_t maxDim, double acceptanceThreshold, OldDIISmode_type mode)
{
  m_maxDim = maxDim;
  ...
  Reset();
  if (m_verbosity>1)
    xout << "m_OldDIISmode set to "<<m_OldDIISmode<<" in setOptions"<<std::endl;
  if (m_verbosity>1)
...
void OldDIIS::LinearSolveSymSvd(Eigen::VectorXd& Out, const Eigen::MatrixXd& Mat, const Eigen::VectorXd& In, unsigned int nDim, double Thr)
{
...
//  if (pos != std::string::npos)  throw std::logic_error("parsing of weight not implemented yet"); //FIXME
  ...
  if ( m_iNext == 0 && fThisResidualDot > m_acceptanceThreshold )
    // current vector is to be considered too wrong to be useful for OldDIIS
  ...
  // ^- note: this is the OldDIIS dimension--the actual matrices and vectors have
...
//  }
  ...
  // build actual OldDIIS system for the subspace used.
  Eigen::VectorXd
      Rhs(nDim+1),
      Coeffs(nDim+1);
  Eigen::MatrixXd
...
//  xout << "Rhs:"<<std::endl<<Rhs<<std::endl;
  ...
  //     bool
  //         PrintOldDIISstate = true;
  //     if ( PrintOldDIISstate ) {
  //         std::ostream &xout = xout;
  //         xout << "  iUsedVecs: "; for ( uint i = 0; i < nDim; ++ i ) xout << " " << iUsedVecs[i]; xout << std::endl;
  //         PrintMatrixGen( xout, m_ErrorMatrix.data(), nMaxDim(), 1, nMaxDim(), m_ErrorMatrix.nStride(), "OldDIIS-B (resident)" );
  //         PrintMatrixGen( xout, B.data(), nDim+1, 1, nDim+1, B.nStride(), "OldDIIS-B/I" );
  //         PrintMatrixGen( xout, Rhs.data(), 1, 1, nDim+1, 1, "OldDIIS-Rhs" );
  //         PrintMatrixGen( xout, Coeffs.data(), 1, 1, nDim+1, 1, "OldDIIS-C" );
  //         xout << std::endl;
  //     }
...
//            xout << "add solution "<<iUsedVecs[k]<<m_solutions[iUsedVecs[k]].front()<<" with factor "<<Coeffs[k]<<std::endl;
  ...
  if (m_verbosity>2) {
      xout << "OldDIIS.extrapolate() final extrapolated solution: "<<solution.front()<<std::endl;
      xout << "OldDIIS.extrapolate() final extrapolated residual: "<<residual.front()<<std::endl;
    }
}

void OldDIIS::FindUsefulVectors(uint *iUsedVecs, uint &nDim, double &fBaseScale, uint iThis)
{
...
void OldDIIS::test(int verbosity,
                size_t maxDim, double acceptanceThreshold, OldDIISmode_type mode, double difficulty)
{
  ...
  if (verbosity>=0) xout << "Test OldDIIS::solver, difficulty="<<difficulty<<std::endl;
  d.Reset();
  (*x.front())[0]=(*x.front())[1]=1-difficulty; // initial guess
