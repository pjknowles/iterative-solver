...
TEST(SimpleVector, dot_product) {
  ...
  for (size_t i = 0; i < 4; i++) {
    ...
    for (auto j = 0; j < 4; j++) {
      ...
      //      std::cout << i << "," << j << "v0.v0: " << v0.dot(v0) << ": " << ans << ": " << v0.dot(v0) - ans <<
      //      std::endl; std::cout << i << "," << j << "v1.v0: " << v1.dot(v0) << ": " << ans << ": " << v1.dot(v0) -
      //      ans << std::endl; std::cout << i << "," << j << "v2.v0: " << v2.dot(v0) << ": " << ans << ": " <<
      //      v2.dot(v0) - ans << std::endl; std::cout << i << "," << j << "v2.v1: " << v2.dot(v1) << ": " << ans << ":
      //      " << v2.dot(v1) - ans << std::endl; std::cout << i << "," << j << ": " << v2.dot(v1) << "=="
      //                << v0.size() * (2 * v0.size() - 1) * (2 * v0.size() + 1) / 3 << std::endl;;
      //      std::cout << i << "," << j << ": " << v2.dot(v2) << "==" << v0.size() * (2 * v0.size() - 1) * (2 *
      //      v0.size() + 1) / 3 << std::endl;; result &= std::fabs(v2.dot(v2) - double(v0.size() * (2 * v0.size() - 1)
      //      * (2 * v0.size() + 1) / 3)) < 1e-1; result &= std::fabs(v2.dot(v1) - double(v0.size() * (2 * v0.size() -
      //      1) * (2 * v0.size() + 1) / 3)) < 1e-1;
