...
using v = linalg::OutOfCoreArray<double>;

static std::stack<std::unique_ptr<linalg::Base<v> > > instances;
...
#endif
  instances.push(std::make_unique<linalg::LinearEigensystem<v> >(linalg::LinearEigensystem<v>(profiler)));
...
IterativeSolverLinearEquationsInitialize(size_t n,
  ...
  for (size_t root = 0; root < nroot; root++) {
    ...
    instances.push(std::make_unique<linalg::LinearEquations<v> >(rr,aughes));
...
#endif
#endif
  instances.push(std::make_unique<linalg::DIIS<v> >(linalg::DIIS<v>()));
  auto& instance = instances.top();
  instance->m_dimension = n;
  instance->m_thresh = thresh;
  instance->m_maxIterations = maxIterations;
...
#endif
  if (*algorithm)
    instances.push(std::make_unique<linalg::Optimize<v> >(linalg::Optimize<v>(algorithm,
                                                                                                minimize != 0)));
  else
    instances.push(std::make_unique<linalg::Optimize<v> >(linalg::Optimize<v>()));
  auto& instance = instances.top();
  instance->m_dimension = n;
  instance->m_roots = 1;
  instance->m_thresh = thresh;
...
extern "C" int IterativeSolverAddValue(double* parameters, double value, double* action, int sync) {
  ...
  auto result = static_cast<linalg::Optimize<v>*>(instance.get())->addValue(ccc, value, ggg) ? 1 : 0;
#ifdef HAVE_MPI_H
  if (sync) {
